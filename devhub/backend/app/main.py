...
async def sync_active_projects_once():
    db = SessionLocal()
    try:
        projects = list(db.scalars(select(Project).where(Project.active == True)).all())
...
async def create_project_from_url(payload: ProjectFromUrl, db: Session = Depends(get_db)):
    ...
    p = Project(
        ...
        repository_description=data.get("description"),
        repository_visibility=data.get("visibility"),
        default_branch=data["default_branch"],
        roadmap_path=payload.roadmap_path or data.get("roadmap_path") or "ROADMAP.md",
        changelog_path=payload.changelog_path or data.get("changelog_path") or "CHANGELOG.md",
        active=True,
...
async def refresh_all(db: Session = Depends(get_db)):
    ...
    for p in db.scalars(select(Project).where(Project.active == True)).all():
        try:
            await sync_project_record(p, db)
            result.append({"id": p.id, "status": "Synced"})
        except Exception as exc:
...
def sync_summary(db: Session = Depends(get_db)):
    projects = list(db.scalars(select(Project).where(Project.active == True)).all())
    failed = [p for p in projects if p.github_sync_status == "Failed"]
    refreshed = [p.github_refreshed_at for p in projects if p.github_refreshed_at]
    return {
        "active_projects": len(projects),
        "failed_projects": len(failed),
        "status": "Operational" if not failed else "Degraded",
        "last_successful_sync": max(refreshed).isoformat() if refreshed else None,
        "interval_seconds": BACKGROUND_SYNC_SECONDS,
    }
...
async def upload_project_logo(project_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if file.content_type not in ALLOWED_LOGO_TYPES:
        raise HTTPException(415, "Logo must be SVG, PNG, WebP or JPEG")
...
def project_logo(project_id: int, db: Session = Depends(get_db)):
    p = project_or_404(db, project_id)
    if not p.logo_path:
        raise HTTPException(404, "Project logo not configured")
    path = (PROJECT_LOGO_DIR / Path(p.logo_path).name).resolve()
    if PROJECT_LOGO_DIR.resolve() not in path.parents or not path.exists():
        raise HTTPException(404, "Project logo missing")
...
async def roadmap(project_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if text is None:
            raise HTTPException(404, "Configured roadmap file was not found")
        ...
        raise HTTPException(502, f"Roadmap retrieval failed: {exc}")
...
def list_register(project_id: int | None = None, db: Session = Depends(get_db)):
    ...
    if project_id:
        stmt = stmt.where(RegisterItem.project_id == project_id)
...
def create_item(payload: RegisterItemCreate, db: Session = Depends(get_db)):
    ...
    for c in payload.criteria:
        db.add(AcceptanceCriterion(item_id=item.id, **c.model_dump()))
    db.commit()
...
def add_criterion(item_id: int, payload: dict, db: Session = Depends(get_db)):
    ...
    c = AcceptanceCriterion(item_id=item_id, description=text, sort_order=int(payload.get("sort_order", 0)))
    db.add(c)
    db.commit()
    db.refresh(c)
...
async def upload_attachments(item_id: int, files: list[UploadFile] = File(...), db: Session = Depends(get_db)):
    ...
    for f in files:
        if f.content_type not in ALLOWED_TYPES:
            raise HTTPException(415, f"Unsupported attachment type: {f.content_type}")
        data = await f.read(MAX_UPLOAD_BYTES + 1)
        if len(data) > MAX_UPLOAD_BYTES:
            raise HTTPException(413, "Attachment exceeds 100 MB limit")
        safe = re.sub(r"[^A-Za-z0-9._-]", "_", Path(f.filename or "file").name)
        stored = f"{uuid.uuid4().hex}_{safe}"
        (UPLOAD_DIR / stored).write_bytes(data)
        a = Attachment(item_id=item_id, original_name=safe, stored_name=stored, content_type=f.content_type or "application/octet-stream", size_bytes=len(data))
        db.add(a)
        db.flush()
...
def create_release(payload: ReleaseCreate, db: Session = Depends(get_db)):
    ...
    r = Release(project_id=payload.project_id, planned_version=payload.planned_version, notes=payload.notes)
    db.add(r)
    db.flush()
    for item_id in payload.item_ids:
        item = item_or_404(db, item_id)
        if item.project_id != payload.project_id:
            raise HTTPException(422, "Release items must belong to the selected project")
...
async def release_prompt(release_id: int, db: Session = Depends(get_db)):
    r = db.scalar(select(Release).options(selectinload(Release.scope).selectinload(ReleaseItem.item).selectinload(RegisterItem.criteria)).where(Release.id == release_id))
    if not r:
        raise HTTPException(404, "Release not found")
    p = project_or_404(db, r.project_id)
    try:
        roadmap_text = await GitHubService().file_text(p.github_owner, p.github_repo, p.roadmap_path, p.default_branch)
    except Exception:
        roadmap_text = None
    return {"prompt": build_release_prompt(p, r, [x.item for x in r.scope], roadmap_text)}


@app.post("/api/releases/{release_id}/reconcile")
async def reconcile_release(release_id: int, db: Session = Depends(get_db)):
    r = db.get(Release, release_id)
    if not r:
        raise HTTPException(404, "Release not found")
    p = project_or_404(db, r.project_id)
    try:
        data = await GitHubService().release_state(p.github_owner, p.github_repo)
        if data:
            r.actual_version = data.get("tag_name")
            r.release_url = data.get("html_url")
            r.release_at = GitHubService.parse_github_datetime(data.get("published_at") or data.get("created_at"))
            r.status = "Reconciled"
            db.commit()
            db.refresh(r)
        return {"release": ReleaseOut.model_validate(r), "github": data}
    except Exception as exc:
        raise HTTPException(502, f"GitHub reconciliation failed: {exc}")


@app.post("/api/releases/{release_id}/test/{criterion_id}")
def update_test_result(release_id: int, criterion_id: int, payload: TestResultUpdate, db: Session = Depends(get_db)):
    if payload.status not in TEST_STATUSES:
        raise HTTPException(422, "Invalid test status")
    r = db.get(Release, release_id)
    if not r:
        raise HTTPException(404, "Release not found")
    ...
    if not result:
        result = AcceptanceTestResult(release_id=release_id, criterion_id=criterion_id)
        db.add(result)
